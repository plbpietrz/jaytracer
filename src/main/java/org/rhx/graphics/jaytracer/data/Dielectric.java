...
public class Dielectric implements Material {
    ...
    private boolean refract(Vec3 v, Vec3 n, float ni_over_nt, Out<Vec3> refracted) {
        Vec3 uv = Vec3.unit(v);
    ...
    private float schlick(final float cosine, final float refIdx) {
        float r0 = (1 - refIdx) / (1 + refIdx);
        r0 = r0 * r0;
        return (r0 + (1 - r0) * (float)Math.pow((1 - cosine), 5f));
    }

    public static Dielectric of(final float refractionIdx) {
